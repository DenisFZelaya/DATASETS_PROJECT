...
def get_images_bing(query, folder_name, max_images=100):
    ...
    # Abre Bing Images
    driver.get('https://www.google.hn/imghp?hl=en&ogbl')
    ...
    while downloaded_images < max_images:
        ...
        div_containers = driver.find_elements(By.CLASS_NAME, 'GyAeWb.gIatYd')
    ...
    for idx, url in enumerate(urls_imagenes):
        try:
            ...
            if url.startswith('data:image'):  # Identificar Base64
                header, encoded = url.split(',', 1)
                data = base64.b64decode(encoded)
                with open(image_name, 'wb') as f:
                    f.write(data)
                print(f"Imagen en Base64 guardada como {image_name}")
            else:  # URL normal
